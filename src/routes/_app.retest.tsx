...
function RetestPage() {
  ...
  const years = useMemo(
    () =>
      Array.from(new Set(items.map((r) => r.tax_year ?? "").filter(Boolean)))
        .sort()
        .reverse(),
    [items],
  );
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground inline-flex items-center gap-2">
            {isAdmin
              ? "Assign testing tasks to agents and track progress."
      ...
      {error && (
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4">
          <p className="inline-flex items-center gap-2 text-sm font-medium">
            <AlertCircle className="h-4 w-4" /> Unable to load task assignment data. Please try
            again.
          </p>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
              <span>
                {visible.length} of {items.length} task{items.length === 1 ? "" : "s"}
              </span>
        ...
        <CardContent className="p-0">
          {loading && visible.length === 0 ? (
            ...
            <p className="p-6 text-sm text-muted-foreground">
              {items.length === 0
                ? "No tasks in this environment yet."
                      ...
                      <TableCell>
                        {isAdmin ? (
                          <Select
                            value={r.priority}
                            onValueChange={async (v) => {
                              const res = await updateAssignment(r.id, {
                                priority: v as RetestPriority,
                              });
                              if (!res.ok) toast.error(res.error);
                            }}
                          >
