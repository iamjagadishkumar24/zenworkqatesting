...
import { MODULE_OPTIONS, TAX_YEARS, DEFAULT_TAX_YEAR, getModuleCatalog } from "@/lib/qa/constants";
...
export function AssignTaskDialog({
  ...
  const [selectedAgents, setSelectedAgents] = useState<Set<string>>(
    () => new Set(defaultAgent ? [defaultAgent] : []),
  );
  ...
  useEffect(() => {
    ...
    listForms({ data: { module: moduleSel } })
      .then((rows) => {
        if (!cancelled) setServerForms(rows ?? []);
      })
      .catch(() => {
        if (!cancelled) setServerForms([]);
      })
      .finally(() => {
  ...
  const [scopeError, setScopeError] = useState<{ message: string; offenders: string[] } | null>(
    null,
  );
  ...
  useEffect(() => {
    setPreviewPage(1);
  }, [previewQuery, previewSortBy, previewSortDir]);
  ...
  const createdDefect = useMemo(() => {
    ...
    const matches = defects
      .filter(
        (d) =>
          d.title === createdDefectTitle &&
          new Date(d.createdAt).getTime() >= createdDefectAfter - 1000,
      )
  ...
  const createDefectFromScopeError = async () => {
    ...
    const res = await addDefect({
      module: (moduleSel && moduleSel !== ALL_MODULES
        ? moduleSel
        : "Functionality Testing") as never,
  ...
  const submit = async () => {
    ...
    const selected = allForms
      ? []
      : scopedForms.filter((f) => picked.has(f.id)).map((f) => ({ id: f.id, name: f.name }));
    ...
    const recipients = [
      ...targetUsers.filter((u) => !!u.email).map((u) => ({ email: u.email, name: u.name })),
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Assign a testing task to a single agent or to all active agents in the current
              environment ({env ?? "—"}).
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <Input
                ...
                placeholder="e.g. Verify 2290 e-file flow"
              />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
                  ...
                  <div className="mt-2">
                    ...
                    {createdDefect && (
                      <span className="ml-2 text-[11px] text-muted-foreground">
                        ...
                        {" · "}Live status:{" "}
                        <span className="font-medium text-foreground">{createdDefect.status}</span>
              ...
              <div className="mt-2 rounded-md border bg-muted/30 p-2 text-xs">
                <div className="flex items-center justify-between">
                  <div className="font-medium text-foreground">
                    Scope preview — allowed for{" "}
                    {moduleSel === ALL_MODULES ? "All Modules" : moduleSel}
                          ...
                          {moduleSel === ALL_MODULES ? (
                            <tr>
                              <td colSpan={3} className="px-2 py-2 text-muted-foreground">
                                Select a Module / Category to preview its catalog.
                              </td>
                            </tr>
                          ) : previewLoading && previewItems.length === 0 ? (
                            <tr>
                              <td colSpan={3} className="px-2 py-2 text-muted-foreground">
                                Loading allowed forms / features…
                              </td>
                            </tr>
                            ...
                            <tr>
                              ...
                              >
                                {previewQuery ? (
                                  ...
                                  <>
                                    No forms / features are mapped to{" "}
                                    <span className="font-medium text-foreground">{moduleSel}</span>{" "}
                                    yet.
                                  </>
                                )}
                    ...
                    {moduleSel !== ALL_MODULES && previewTotal > PREVIEW_PAGE_SIZE && (
                      <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                        <span>
                          ...
                          {Math.min(previewSafePage * PREVIEW_PAGE_SIZE, previewTotal)} of{" "}
                          {previewTotal}
                        </span>
              ...
              <div
                className={`mt-1 max-h-48 overflow-auto rounded-md border p-2 grid gap-1 sm:grid-cols-2 lg:grid-cols-3 ${allForms ? "opacity-50 pointer-events-none" : ""}`}
              >
                {loadingForms && <p className="text-xs text-muted-foreground">Loading forms…</p>}
                {filtered.map((f) => {
              ...
              <div>
                ...
                {offenderInfo.related.length === 0 ? (
                  <div className="text-xs text-muted-foreground">
                    No defects reference this form / feature yet.
                  </div>
                ) : (
                    ...
                    {offenderInfo.related.slice(0, 20).map((d) => (
                      <li key={d.id} className="rounded border p-2 text-xs">
                        ...
                        <div className="mt-1 text-[11px] text-muted-foreground">
                          Priority {d.priority} · Severity {d.severity} · Updated{" "}
