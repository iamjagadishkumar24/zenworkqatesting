...
export function ExportJobsPanel({ isAdmin }: { isAdmin: boolean }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = () => supabase.from("export_jobs").select("*").order("created_at", { ascending: false }).limit(25)
      .then(({ data }) => { if (!cancelled && data) setJobs(data as Row[]); });
    load();
    const ch = supabase
  ...
  const onRetry = async (id: string) => {
    try { await retry({ data: { jobId: id } }); toast.success("Retry started"); }
      ...
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {jobs.map((j) => (
              <TableRow key={j.id}>
                <TableCell className="text-sm">{j.requested_by_name}<div className="text-xs text-muted-foreground capitalize">{j.role}</div></TableCell>
                ...
                <TableCell><Progress value={j.progress ?? 0} /></TableCell>
            ...
            {jobs.length === 0 && (
              <TableRow><TableCell colSpan={8} className="py-8 text-center text-sm text-muted-foreground">No export jobs yet.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </CardContent>
