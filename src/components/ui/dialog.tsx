...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      // Softer scrim + subtle backdrop blur so the dialog reads as a focused
      // surface above frosted page chrome. Premium app convention (Linear,
      // Notion, Stripe).
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // Layered elevation shadow + hairline border + larger radius. bg-card
        // instead of bg-background so dialogs sit on the same surface ladder
        // as cards in dark mode (background gets darker than card).
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground ring-offset-background cursor-pointer transition-colors hover:bg-muted hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
