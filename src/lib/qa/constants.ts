...
/**
 * U.S. states, the District of Columbia, and U.S. territories.
 * Used by State Filing error reports — one selection per report.
 * Stored on `defects.state` as the USPS 2-letter code.
 */
...
export function isValidUsState(code: string | null | undefined): boolean {
  return !!code && US_STATE_CODES.includes(code);
}
