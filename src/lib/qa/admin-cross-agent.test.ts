...
describe("Admin defect filters", () => {
  ...
  it("filters by tax year", () => {
    expect(
      filterDefectsAdmin(ALL, { taxYear: "2026" })
        .map((d) => d.id)
        .sort(),
    ).toEqual(["D1", "D2"]);
    expect(
      filterDefectsAdmin(ALL, { taxYear: "2025" })
        .map((d) => d.id)
        .sort(),
    ).toEqual(["D3", "D4"]);
  });
  ...
  it("filters by comment presence", () => {
    ...
    expect(
      filterDefectsAdmin(ALL, { hasComments: "no" })
        .map((d) => d.id)
        .sort(),
    ).toEqual(["D1", "D3", "D4"]);
  ...
  it("filters by attachment presence", () => {
    ...
    expect(
      filterDefectsAdmin(ALL, { hasAttachments: "no" })
        .map((d) => d.id)
        .sort(),
    ).toEqual(["D1", "D3", "D4"]);
  });
  ...
  it("filters by retest state", () => {
    ...
    expect(
      filterDefectsAdmin(ALL, { retest: "none" })
        .map((d) => d.id)
        .sort(),
    ).toEqual(["D1", "D4"]);
  });
...
describe("Audit log admin filters", () => {
  ...
  it("filters audit rows by action verb across agents", () => {
    expect(
      filterAuditAdmin(rows, { actionKind: "create" })
        .map((r) => r.action)
        .sort(),
    ).toEqual(["defect.created", "task.created"]);
    expect(
      filterAuditAdmin(rows, { actionKind: "close" })
        .map((r) => r.action)
        .sort(),
    ).toEqual(["defect.closed", "task.completed"]);
    expect(filterAuditAdmin(rows, { actionKind: "reopen" }).map((r) => r.action)).toEqual([
      "defect.reopened",
    ]);
    expect(filterAuditAdmin(rows, { actionKind: "export" }).map((r) => r.action)).toEqual([
      "export.completed",
    ]);
  });
  ...
  it("'any' is a no-op for both record kind and action", () => {
    expect(filterAuditAdmin(rows, { recordKind: "any", actionKind: "any" })).toHaveLength(
      rows.length,
    );
  });
});
