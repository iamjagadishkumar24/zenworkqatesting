...
const RPC_ENDPOINTS: RpcEndpoint[] = [
  ...
  },
  {
    ...
    call: (c) => c.rpc("purge_orphaned_agent_refs") as unknown as Promise<CallResult>,
    matrix: { anon: "deny", agent: "deny", admin: "deny" }, // service_role only
  },
  {
    name: "next_scoped_id",
    call: (c) =>
      c.rpc("next_scoped_id", {
        _kind: "defect",
        _tax_year: "2099",
      }) as unknown as Promise<CallResult>,
...
const TABLE_ENDPOINTS: TableEndpoint[] = [
  ...
  {
    table: "qa_runtime_config_audit",
    selectMatrix: { anon: "deny", agent: "allow", admin: "allow" },
  },
  { table: "agent_invites", selectMatrix: { anon: "deny", agent: "allow", admin: "allow" } },
  {
    table: "retest_pending_assignments",
    selectMatrix: { anon: "deny", agent: "allow", admin: "allow" },
  },
