...
function isAdminGuardRejection(error: { message?: string } | null) {
  if (!error) return false;
  return /only admins?|not authenticated|admin role|forbidden/i.test(error.message ?? "");
}
...
describeIfAgent("SECURITY DEFINER RPCs — authenticated non-admin is rejected by guard", () => {
  ...
  it.each([
    ...
    {
      name: "has_role",
      args: { _user_id: "00000000-0000-0000-0000-000000000000", _role: "admin" as const },
    },
  ...
  ])("non-admin → $name IS allowed (authenticated grant)", async ({ name, args }) => {
    const rpc = client.rpc as unknown as (n: string, a?: unknown) => Promise<{ error: unknown }>;
